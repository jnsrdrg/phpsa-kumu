...
######PARALLEL-LINT###
parallel-lint $proj | tee parallel-lint.log
cat parallel-lint.log
parallel_lint_error=`cat parallel-lint.log | grep "Syntax error found in" | cut -d " " -f 5`
...
cat phpstan.log
phpstan_error=`cat phpstan.log | grep "ERROR" | cut -d " " -f 4`
...
phpcs_error=`cat phpcs.log | grep FOUND | cut -d " " -f 2 `
...
cat local-php-security-checker.log
local_php_security_checker_error=`cat local-php-security-checker.log | grep "packages have known vulnerabilities" | cut -d " " -f 1 `
re='^[0-9]+$'
